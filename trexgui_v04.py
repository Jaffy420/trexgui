...
import time
import psutil
from tkinter import END, Entry, Tk, Frame, Canvas, Label, PhotoImage, Text, WORD, Scale, Button, HORIZONTAL, IntVar, INSERT, messagebox
...
def system_box(canvas2, config, root, combo):
    """ Create Help Box """
    system_txt = Text(canvas2,
                    bd = 0,
                    bg = "#dddddd",
                    highlightthickness = 0,
                    font=('Helvatical bold',12),
    ...
    system_txt.place(x = 602, y = 75,
                  width = 360,
                  height = 370)
    return system_txt


def create_miner_tab_buttons(canvas, root, config, mine_save_clicked, mine_clicked, bat_clicked, vm_clicked):
...
def create_rig_tab_text(canvas2, config, root):
    ...
    rigname_txt = Entry(canvas2,
                        ...
                        bg = "#dddddd",
                        highlightthickness = 0)
    rigname_txt.place(x = 28, y = 212,
                      width = 100.0,
                      height = 40)
    rigname_txt.insert(0, config['Rig_Info']['rig_name'])
    
    _ = canvas2.create_image(220, 232,
                             image = entry02_img)
    password_txt = Entry(canvas2,
                         bd = 0,
                         bg = "#dddddd",
                         highlightthickness = 0)
    password_txt.place(x = 175, y = 212,
                       width = 100.0,
                       height = 40)
    password_txt.insert(0, config['Rig_Info']['password'])
    
    _ = canvas2.create_image(400, 232,
                             image = entry02_img)
    report_interval_txt = Entry(canvas2,
                         bd = 0,
                         bg = "#dddddd",
                         highlightthickness = 0)
    report_interval_txt.place(x = 355, y = 212,
                          width = 100.0,
                          height = 40)
    report_interval_txt.insert(0, config['Rig_Info']['gpu_report'])
    
    _ = canvas2.create_image(75, 390,
                             image = entry02_img)
    temp_color_txt = Entry(canvas2,
                         bd = 0,
                         bg = "#dddddd",
                         highlightthickness = 0)
    temp_color_txt.place(x = 28, y = 370,
                          width = 100.0,
                          height = 40)
    temp_color_txt.insert(0, config['Rig_Info']['temp_color'])
    
    _ = canvas2.create_image(220, 390,
                             image = entry02_img)
    mem_temp_color_txt = Entry(canvas2,
                         bd = 0,
                         bg = "#dddddd",
                         highlightthickness = 0)
    mem_temp_color_txt.place(x = 175, y = 370,
                          width = 100.0,
                          height = 40)
    mem_temp_color_txt.insert(0, config['Rig_Info']['mem_temp_color'])
    
    _ = canvas2.create_image(75, 470,
                             image = entry02_img)
    temp_limit_txt = Entry(canvas2,
                         bd = 0,
                         bg = "#dddddd",
                         highlightthickness = 0)
    temp_limit_txt.place(x = 28, y = 450,
                          width = 100.0,
                          height = 40)
    temp_limit_txt.insert(0, config['Rig_Info']['temp_limit'])
    
    _ = canvas2.create_image(220, 470,
                             image = entry02_img)
    temp_start_txt = Entry(canvas2,
                         bd = 0,
                         bg = "#dddddd",
                         highlightthickness = 0)
    temp_start_txt.place(x = 175, y = 450,
                          width = 100.0,
                          height = 40)
    temp_start_txt.insert(0, config['Rig_Info']['temp_start'])
    
    _ = canvas2.create_image(175, 309,
                             image = entry122_img)
    ...
    email_txt.place(x = 25, y = 289,
                    width = 300,
                    height = 40)
    email_txt.insert(0, config['Rig_Info']['email'])
    
    
    _ = canvas2.create_image(280, 148,
                             image = entry22_img)
    ...
    return rigname_txt, password_txt, location_txt, startup_txt, email_txt, temp_color_txt, mem_temp_color_txt, temp_limit_txt, temp_start_txt, report_interval_txt


def create_rig_tab_buttons(canvas2, root, rig_save_clicked, vm_clicked):
    """ Create Rig Tab Buttons """
    root.img02 = img02 = PhotoImage(file = "img1.png")
    root.img03 = img03 = PhotoImage(file = "img3.png")
    ...
    rig_save.place(x = 325, y = 480,
                   width = 250,
                   height = 59)
    
    vm_click = Button(canvas2,
                      image = img03,
                      borderwidth = 0,
                      highlightthickness = 0,
                      command = vm_clicked,
...
def main():
    ...
    def rig_save_clicked():
        ...
        config.set('Rig_Info', 'temp_color', temp_color_txt.get())
        config.set('Rig_Info', 'mem_temp_color', mem_temp_color_txt.get())
        config.set('Rig_Info', 'temp_limit', temp_limit_txt.get())
        config.set('Rig_Info', 'temp_start', temp_start_txt.get())
        config.set('Rig_Info', 'gpu_report', report_interval_txt.get())
    ...
    def vm_clicked():
        is_admin = messagebox.askyesno(title='Requires Admin and Restart',
                                       message='Requires Run as Admin, Continue?')
        if is_admin:
            update_mem = messagebox.askyesno(title='Suggested Virtual Memory',
                                             message='Suggested Virtual Memory: {}MB, Continue?'.format(recommended))
            if update_mem:
                print('executing pagefile')
                #total_gpu_size = 10000
                print('start cmd /k wmic computersystem where name="%computername%" set AutomaticManagedPagefile=false && wmic pagefileset where name="C:\\\pagefile.sys" set InitialSize={recommended},MaximumSize={recommended}'.format(recommended=recommended))
                os.system('start cmd /k "wmic computersystem where name="%computername%" set AutomaticManagedPagefile=false && wmic pagefileset where name="C:\\\pagefile.sys" set InitialSize={recommended},MaximumSize={recommended}"'.format(recommended=recommended))
                #os.system('start cmd /k )
                restart = messagebox.askyesno(title='Restart Now?',
                                             message='Are you sure you want to restart?')
                if restart:
                    os.system('start cmd shutdown -r -t 00')
    ...
    def create_binds():
        ...
        temp_color_txt.bind("<KeyRelease>", lambda _ : update_bat())
        mem_temp_color_txt.bind("<KeyRelease>", lambda _ : update_bat())
        temp_limit_txt.bind("<KeyRelease>", lambda _ : update_bat())
        temp_start_txt.bind("<KeyRelease>", lambda _ : update_bat())
        report_interval_txt.bind("<KeyRelease>", lambda _ : update_bat())
    ...
    def update_bat():
        ...
        if temp_color_txt.get() != '':
            new_bat = '{} --temperature-color {}'.format(new_bat, temp_color_txt.get())   
        if mem_temp_color_txt.get() != '':
            new_bat = '{} --temperature-color-mem {}'.format(new_bat, mem_temp_color_txt.get())
        if temp_limit_txt.get() != '':
            new_bat = '{} --temperature-limit {}'.format(new_bat, temp_limit_txt.get())
        if temp_start_txt.get() != '':
            new_bat = '{} --temperature-start {}'.format(new_bat, temp_start_txt.get())
        if report_interval_txt.get() != '' and report_interval_txt.get() != '30':
            new_bat = '{} --gpu-report-interval {}'.format(new_bat, report_interval_txt.get())
    ...
    total_gpu_size = 0
    for GPU in GPUs:
        total_gpu_size += GPU.memoryTotal
    ...
    start_mine, mine_config, create_bat, background_nodual, background = create_miner_tab_buttons(canvas, root, config, mine_save_clicked, mine_clicked, bat_clicked, vm_clicked)
    ...
    system_txt = system_box(canvas2, config, root, combo)
    ...
    rigname_txt, password_txt, location_txt, startup_txt, email_txt, temp_color_txt, mem_temp_color_txt, temp_limit_txt, temp_start_txt, report_interval_txt = create_rig_tab_text(canvas2, config, root)
    create_rig_tab_buttons(canvas2, root, rig_save_clicked, vm_clicked)
    ...
    #print(round(hdd.free/1024/1024, 2))
    #print(psutil.virtual_memory().free/1024/1024)
    #print(psutil.swap_memory().free/1024/1024)
    if hdd.free/1024/1024 < total_gpu_size:
        recommended = round(((hdd.free/1024/1024) + round((psutil.swap_memory().total-psutil.virtual_memory().total)/1024/1024)) - 2500)
    else:
        recommended = total_gpu_size
    system_txt.insert(INSERT, 'Installed Memory {} MB\n'.format(round(psutil.virtual_memory().total/1024/1024)))
    system_txt.insert(INSERT, 'Virtual Memory {} MB\n'.format(round((psutil.swap_memory().total-psutil.virtual_memory().total)/1024/1024)))
    system_txt.insert(INSERT, 'Total HDD Space {} MB\n'.format(round(hdd.free/1024/1024, 2)))
    system_txt.insert(INSERT, 'Total GPU Size: {} MB\n'.format(total_gpu_size))
    system_txt.insert(INSERT, 'Recommended Virtual Memory {} MB\n'.format(recommended))
    system_txt.insert(INSERT, '\nGPUs\n')
    
    for GPU in GPUs:
        system_txt.insert(INSERT, '{} {}\n'.format(GPU.name, GPU.memoryTotal))
    system_txt.configure(state='disabled')
